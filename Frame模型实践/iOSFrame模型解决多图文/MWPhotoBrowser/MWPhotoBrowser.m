 ...
 *  设置Padding
 *
 *  @param paddingMultiples <#paddingMultiples description#>
 */
-(void)setPaddingMultiples:(BOOL)paddingMultiples{
    if (!paddingMultiples) {
        _paddingNumber = 1;
    }else{
        _paddingNumber = 0;
...
- (void)performLayout {
    ...
    if ([self.navigationController.viewControllers objectAtIndex:0] == self) {
        ...
        self.navigationItem.leftBarButtonItem = [[UIBarButtonItem alloc]initWithTitle:NSLocalizedString(@"Cancel", nil) style:UIBarButtonItemStylePlain target:self action:@selector(dismissSelf)];
...
- (void)reloadData {
    //remove right item button.--added by xiaobin
...
- (void)tilePages {
	...
	NSInteger iFirstIndex = (NSInteger)floorf((CGRectGetMinX(visibleBounds)+PADDING*2*_paddingNumber) / CGRectGetWidth(visibleBounds));
	NSInteger iLastIndex  = (NSInteger)floorf((CGRectGetMaxX(visibleBounds)-PADDING*2*_paddingNumber-1) / CGRectGetWidth(visibleBounds));
...
- (CGRect)frameForPagingScrollView {
    ...
    frame.origin.x -= PADDING*_paddingNumber;
    frame.size.width += (2 * PADDING*_paddingNumber);
...
- (CGRect)frameForPageAtIndex:(NSUInteger)index {
    ...
    pageFrame.size.width -= (2 * PADDING*_paddingNumber);
    pageFrame.origin.x = (bounds.size.width * index) + PADDING*_paddingNumber;
...
- (void)jumpToPageAtIndex:(NSUInteger)index animated:(BOOL)animated {
	...
	if (index < [self numberOfPhotos]) {
		CGRect pageFrame = [self frameForPageAtIndex:index];
        [_pagingScrollView setContentOffset:CGPointMake(pageFrame.origin.x - PADDING*_paddingNumber, 0) animated:animated];
		[self updateNavigation];
	}
...
#pragma mark - reloadGirdData
- (void)reloadGirdData{
    [_gridController refreshData];
}
            ...
            if ([self.delegate respondsToSelector:@selector(photoBrowser:actionButtonPressedForPhotoAtIndex:)]) {
                ...
                if (!self.shareActivity) {
                    if(self.displayShareButton){
                        self.shareActivity = [[HYActivityView alloc]initWithTitle:@"操作" referView:self.view];
                        
                        //横屏会变成一行6个, 竖屏无法一行同时显示6个, 会自动使用默认一行4个的设置.
                        self.shareActivity.numberOfButtonPerLine = 4;
                        
                        ButtonView *bv = [[ButtonView alloc]initWithText:@"微信分享" image:[UIImage imageNamed:@"share_platform_wechat"] handler:^(ButtonView *buttonView){
                            [self sharedSelected:Tencent_weixin_friend];
                        }];
                        [self.shareActivity addButtonView:bv];
                        
                        bv = [[ButtonView alloc]initWithText:@"朋友圈分享" image:[UIImage imageNamed:@"share_platform_wechat_moment"] handler:^(ButtonView *buttonView){
                            [self sharedSelected:Tencent_weixin_monent];
                        }];
                        [self.shareActivity addButtonView:bv];
                        
                        bv = [[ButtonView alloc]initWithText:@"微博分享" image:[UIImage imageNamed:@"share_platform_sina"] handler:^(ButtonView *buttonView){
                            [self sharedSelected:Sina_weibo];
                        }];
                        [self.shareActivity addButtonView:bv];
                        
                        
                        bv = [[ButtonView alloc]initWithText:@"QQ好友" image:[UIImage imageNamed:@"share_platform_qqfriend"] handler:^(ButtonView *buttonView){
                              [self sharedSelected:Tencent_qq_friend];
                        }];
                        [self.shareActivity addButtonView:bv];
                        
                        bv = [[ButtonView alloc]initWithText:@"QQ空间" image:[UIImage imageNamed:@"share_platform_qqZone"] handler:^(ButtonView *buttonView){
                              [self sharedSelected:Tencent_qq_QZone];
                        }];
                        [self.shareActivity addButtonView:bv];
                        
                        bv = [[ButtonView alloc]initWithText:@"存到相册" image:[UIImage imageNamed:@"share_save_local"] handler:^(ButtonView *buttonView){
                            [self sharedSelected:Phone_gallery];
                        }];
                        [self.shareActivity addButtonView:bv];
                        
                        bv = [[ButtonView alloc]initWithText:@"删除" image:[UIImage imageNamed:@"action_type_delete"] handler:^(ButtonView *buttonView){
                            [self sharedSelected:Delete_record];
                        }];
                        [self.shareActivity addButtonView:bv];
                    }else{
                        
                        self.shareActivity = [[HYActivityView alloc]initWithTitle:@"保存到" referView:self.view];
                        
                        //横屏会变成一行6个, 竖屏无法一行同时显示6个, 会自动使用默认一行4个的设置.
                        self.shareActivity.numberOfButtonPerLine = 1;

                        
                        ButtonView *bv = [[ButtonView alloc]initWithText:@"手机相册" image:[UIImage imageNamed:@"share_save_local"] handler:^(ButtonView *buttonView){
                            [self sharedSelected:Phone_gallery];
                        }];
                        [self.shareActivity addButtonView:bv];
                        
                        if (self.displaySaveRecordButton) {
                            
                            self.shareActivity.numberOfButtonPerLine = 2;
                            
                            bv = [[ButtonView alloc]initWithText:@"成长记" image:[UIImage imageNamed:@"action_downLoad_record"] handler:^(ButtonView *buttonView){
                                if ([self.delegate respondsToSelector:@selector(photoBrowser:actionButtonPressedForSaveToRecordAtIndex:)]) {
                                    [self.delegate photoBrowser:self actionButtonPressedForSaveToRecordAtIndex:_currentPageIndex];
                                }
                            }];
                            [self.shareActivity addButtonView:bv];
                        }
...
- (void)sharedSelected:(ShareType)type {
    if(type == Phone_gallery){//save image to local
        [self savePhoto];
    }else if(type == Delete_record){
        if ([self.delegate respondsToSelector:@selector(photoBrowser:actionButtonPressedForShareAtIndex:WithShareType:)]) {
            [self.delegate photoBrowser:self actionButtonPressedForShareAtIndex:_currentPageIndex WithShareType:type];
        }
    }else{
        if ([self.delegate respondsToSelector:@selector(photoBrowser:actionButtonPressedForShareAtIndex:WithShareType:)]) {
            if (type==Tencent_qq_QZone||type==Tencent_qq_friend) {
                [self.delegate photoBrowser:self actionButtonPressedForShareAtIndex:_currentPageIndex WithShareType:type];
            }else{
                // Let delegate handle things
                [self showProgressHUDWithMessage:[NSString stringWithFormat:@"%@\u2026" , @"正在加载"]];
